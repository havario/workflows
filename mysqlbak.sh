...
gamedb1_bak() {
    ...
    while read -r DB; do
        DATABASES_1+=("$DB")
    done < <(mysql -h "${GAMEDB_1[MYSQL_IP_GAMEDB1]}" \
            -u "${GAMEDB_1[MYSQL_USER_GAMEDB1]}" \
            -p"${GAMEDB_1[MYSQL_PASSWD_GAMEDB1]}" \
    ...
    for database in "${DATABASES_1[@]}"; do
        ...
        -h "${GAMEDB_1[MYSQL_IP_GAMEDB1]}" \
        -P "${GAMEDB_1[MYSQL_PORT_GAMEDB1]}" \
        -u "${GAMEDB_1[MYSQL_USER_GAMEDB1]}" \
        -p"${GAMEDB_1[MYSQL_PASSWD_GAMEDB1]}" \
