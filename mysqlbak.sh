...
gamedb1_bak() {
    ...
    while read -r DB; do
        DATABASES_1+=("$DB")
    done < <(mysql -h "${GAMEDB_1[MYSQL_IP_GAMEDB1]}" -u "${GAMEDB_1[MYSQL_USER_GAMEDB1]}" -p"${GAMEDB_1[MYSQL_PASSWD_GAMEDB1]}" -e "SHOW DATABASES;" 2>/dev/null | grep -Ev "(Database|information_schema|mysql|performance_schema|sys)")
